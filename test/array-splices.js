...
describe("observe arrays", function(){
	it('should report changes', function(){
		...
		var result;
		var observedArray = observeArraySlices(
			c(['a', 'b', 'c']),
			function(changes) {
				result = changes;
			});
		...
		observedArray[1] = 'z';
		assert.deepEqual( result, [{ type: 'splice', index: 1, removedCount: 1, added: [ 'z' ] }] );

		observedArray.push('last');
		assert.deepEqual( result, [{ type: 'splice', index: 3, removedCount: 0, added: [ 'last' ] }] );
