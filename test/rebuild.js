...
describe("Re build shape analysis", function(){
  ...
  it("Test rebuild with hybrid analysis", function(){ 
      ...
      () => {
        ...
        if (state.value === 2) {
          result = new Aggregate({
            ...
            b: new Aggregate({
              a: new Primitive({value: 43, key: "locked-identity"})
            })
          })
        }
      },
    ...
    result.a.decorate = "someValue";
    log(result);
    log(result.a.causality.buildId);

    logg();
    state.value = 2; // Trigger rebulid
    
    log(result);
    assert.equal(result.b.a.decorate, "someValue"); // Still decorated! No build ids were used!
